...
class OptimizedMarkdownChunkLoader:
    ...
    def _process_indicator_section(self, section: str, indicator_title: str, start_id: int) -> List[Dict[str, Any]]:
        ...
        # 提取体检方法部分（如果存在）
        method_pattern = r'# 【体检方法】\s*(.*?)(?=\n# \[|$)'
        method_match = re.search(method_pattern, section, re.DOTALL)
        method_content = method_match.group(1).strip() if method_match else ""
        ...
        chunks.append({
            ...
            "metadata": {
                ...
                "method_content": method_content,  # 添加体检方法内容
                "keywords": keywords,  # 添加关键词
                "chunk_type": "indicator_complete"
            }
        })
        
        # 为每个图片创建独立的chunk，但保持与指标的关联
        for i, img_path in enumerate(image_matches):
            ...
            chunks.append({
                "type": "image",
                "content": None,  # 图片内容待VLM处理
                "metadata": {
                    "source": self.md_path,
                    "chunk_id": start_id + 1 + i,
                    "indicator_title": indicator_title,
                    "img_path": abs_img_path,
                    "context": section,  # 图片块包含完整的指标内容作为上下文
